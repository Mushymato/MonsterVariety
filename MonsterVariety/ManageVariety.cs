...
internal static class ManageVariety
{
    ...
    private static readonly ConditionalWeakTable<Monster, MonsterLightWatcher> monsterCtx = [];
    ...
    {
        foreach ((_, MonsterLightWatcher? mlw) in monsterCtx)
        {
            mlw?.Deactivate();
        }
        monsterCtx.Clear();
    ...
    {
        if (value is Monster monster && monsterCtx.TryGetValue(monster, out MonsterLightWatcher? mlw))
        {
            mlw.Deactivate();
            monsterCtx.Remove(monster);
        }
    }
    ...
    private static void RemoveExcludedDrops(Monster monster, List<string> excludeDrops)
    {
        foreach (string dropId in excludeDrops)
        {
            if (ItemRegistry.GetData(dropId) is not ParsedItemData itemData)
                continue;
            monster.objectsToDrop.RemoveWhere(itemId =>
                itemId.EqualsIgnoreCase(itemData.ItemId) || itemId.EqualsIgnoreCase(itemData.QualifiedItemId)
            );
        }
    }
    ...
    private static void ApplyMonsterVariety(Monster monster)
    {
        ...
        if (!monster.modData.TryGetValue(ModData_AppliedVariety, out string textureName))
        {
            ...
            if (validVariety.Count > 0)
            {
                ...
                chosenVariety.InfoOverrides?.Apply(monster);
                ...
                if (chosenVariety.ExcludeDrops != null)
                    RemoveExcludedDrops(monster, chosenVariety.ExcludeDrops);
        ...
        {
            MonsterLightWatcher watcher = monsterCtx.GetValue(monster, MonsterLightWatcher.Create);
